...
public class BenefitAssignService {
	...
	// 交易态势中交易量柱状图、饼状图
	public List<BenefitAssign> listChartPie(String searchKeyword,
		...
		// 搜索日期1和2都不为空，搜索关键词空
				...
				&& (searchKeyword.equals("") || searchKeyword.isEmpty())) {
			where = " WHERE bf.jysj>='" + searchDate1 + "'" + " and bf.jysj<='"
					+ searchDate2 + "'";
		}
		...
		// 搜索日期不为空，搜索关键词不为空
			...
			where = " WHERE bf.jyqy like '%" + searchKeyword + "%'"
					+ "and bf.jysj>='" + searchDate1 + "'" + "and bf.jysj<='"
					+ searchDate2 + "'";
		}
		// 搜索日期空，搜索关键词不为空
		else if (searchDate1.equals("") && searchDate2.equals("")
		...
		try {
			org.hibernate.Query query = sessionFactory.getCurrentSession()
					.createQuery(hql + where + " GROUP BY bf.jysj,bf.jyqy");
			results = query.list();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	// 交易态势中交易次数柱状图、饼状图
	public List<BenefitAssign> listChartNumber(String searchKeyword,
			String searchDate1, String searchDate2) {
		...
		try {
			org.hibernate.Query query = sessionFactory.getCurrentSession()
					.createQuery(hql + where + " GROUP BY bf.jyqy");
			results = query.list();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	// 交易态势中交易价格折线图
	public List<BenefitAssign> listChartLine(String searchKeyword,
			String searchDate1, String searchDate2) {
	...
	// 交易态势信息查询
	public Map<String, Object> listTableTaiFind(String page, String start,
			String limit, String searchKeyword1, String searchDate1,
			String searchDate2) {
		...
		// 搜索日期1不为空，日期2为空，搜索关键词空
		if (!searchDate1.equals("") && searchDate2.equals("")
				&& (searchKeyword1.equals("") || searchKeyword1.isEmpty())) {
	...
	// 交易态势预警信息饼状图、柱状图显示
	public List<BenefitAlarmtai> listChartTaishiAlarm(String searchKeyword,
			String searchDate1, String searchDate2) {
		...
		// 搜索日期1和2都不为空，搜索关键词空
				...
				&& (searchKeyword.equals("") || searchKeyword.isEmpty())) {
			where = " WHERE bf.cxsj>='" + searchDate1 + "'" + " and bf.cxsj<='"
					+ searchDate2 + "'";
		}
		// 搜索日期1不为空，日期2为空，搜索关键词不为空
		else if (!searchDate1.equals("") && searchDate2.equals("")
		...
		// 搜索日期不为空，搜索关键词不为空
				...
				&& !(searchKeyword.equals("") || searchKeyword.isEmpty())) {
			where = " WHERE bf.jyqy like '%" + searchKeyword + "%'"
					+ "and bf.cxsj>='" + searchDate1 + "'" + "and bf.cxsj<='"
					+ searchDate2 + "'";
		}
		// 搜索日期空，搜索关键词不为空
		else if (searchDate1.equals("") && searchDate2.equals("")
		...
		// // String
		// shql="select new map(deposit.yclx as yctype,count(deposit.yclx) as yccount) "+hql;

		List<BenefitAlarmtai> results = null;
		try {
			org.hibernate.Query query = sessionFactory.getCurrentSession()
					.createQuery(hql + where + " GROUP BY bf.yclx");
			results = query.list();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	// 交易态势预警信息表格显示查询
	public Map<String, Object> listTableTaiAlarm(String page, String start,
			String limit, String searchKeyword, String searchDate1,
			String searchDate2) {
	...
	public void addShouRegister(HttpServletRequest request,
		...
		MultipartFile multipartFile = multipartHttpservletRequest
				.getFile(filename); // 获得上传图片名称
		String originalFileName = multipartFile.getOriginalFilename();// 获得上传图片新生成的名称

		File file = new File("E:\\eclipsework\\Mytest\\WebContent\\images\\"
				+ request.getParameter("pjh") + "_" + generateFileName() + "");// 上传的文件存放路径
		try {
		...
		try {
			...
			benefitTrack.setJysj(new SimpleDateFormat("yy年MM月dd日")
					.parse(Encoder.encode(request.getParameter("jysj"))));
		...
		benefitTrack.setScsmj(request.getParameter("pjh") + "_"
				+ generateFileName() + "");
	...
	// 收益分配合同信息查询
	public Map<String, Object> listTableShouFind(String page, String start,
			String limit, String searchKeyword1, String searchDate1,
			String searchDate2) {
	...
	// 收益分配资金流动信息查询
	public Map<String, Object> listTableShouFindTrack(String page,
			String start, String limit, String searchKeyword1,
			String searchDate1, String searchDate2) {
		String hql = "";
	...
	// /收益分配预警信息饼状图、柱状图显示
	public List<BenefitAlarmshou> listChartShouyiAlarm(String searchKeyword,
			String searchDate1, String searchDate2) {
	...
	// 收益分配预警信息表格显示查询
	public Map<String, Object> listTableShouAlarm(String page, String start,
			String limit, String searchKeyword, String searchDate1,
			String searchDate2) {
	...
	// 收益分配建设单位信息统计查询
	public Map<String, Object> listTableShouStatJian(String page, String start,
			String limit, String searchKeyword1, String searchDate1,
			String searchDate2) {
		String hql = "";
	...
	// 收益分配农户信息统计查询
	public Map<String, Object> listTableShouStatNong(String page, String start,
			String limit, String searchKeyword1, String searchDate1,
			String searchDate2) {
		String hql = "";
